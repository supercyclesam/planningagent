  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", right: 0, top: 0, width: 480, height: "100vh", background: "#fff", borderLeft: "1px solid #e0ddd8", padding: 32, overflowY: "auto", zIndex: 200, boxShadow: "-4px 0 24px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>{item.tags}</div>
        <p style={{ fontSize: 14, lineHeight: 1.6, color: "#333", marginBottom: 16 }}>{item.desc}</p>
        {item.agent && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: GOLD, margin: "20px 0 8px" }}>Agent</h4><p style={{ fontSize: 14, color: "#333" }}>{item.agent}</p></>)}
        {item.category && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: GOLD, margin: "20px 0 8px" }}>Category</h4><span style={{ display: "inline-block", fontSize: 12, fontWeight: 600, padding: "3px 10px", borderRadius: 4, background: (catColors[item.category] || {}).bg, color: (catColors[item.category] || {}).text }}>{item.category}</span></>)}
        {item.prompt && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: GOLD, margin: "20px 0 8px" }}>System Prompt Essence</h4><p style={{ fontSize: 14, color: "#333", fontStyle: "italic", borderLeft: `3px solid ${GOLD}`, paddingLeft: 12, lineHeight: 1.6 }}>{item.prompt}</p></>)}
        {item.skills && item.skills.length > 0 && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: GOLD, margin: "20px 0 8px" }}>Skills</h4><div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{item.skills.map(s => <span key={s} style={{ display: "inline-block", background: "#f5f5f0", border: "1px solid #e0ddd8", borderRadius: 4, padding: "2px 8px", fontSize: 12 }}>{s}</span>)}</div></>)}
        {item.tools && item.tools.length > 0 && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: GOLD, margin: "20px 0 8px" }}>Tools</h4><div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{item.tools.map(t => <span key={t} style={{ display: "inline-block", background: "#f5f5f0", border: "1px solid #e0ddd8", borderRadius: 4, padding: "2px 8px", fontSize: 12 }}>{t}</span>)}</div></>)}
        {item.scripts && item.scripts.length > 0 && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: GOLD, margin: "20px 0 8px" }}>Scripts</h4><div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{item.scripts.map(s => <span key={s} style={{ display: "inline-block", background: "#FFF3E0", border: "1px solid #FFE0B2", borderRadius: 4, padding: "2px 8px", fontSize: 12, fontFamily: "monospace" }}>{s}</span>)}</div></>)}
        {item.gotchas && item.gotchas.length > 0 && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: "#C62828", margin: "20px 0 8px" }}>Gotchas</h4><ul style={{ paddingLeft: 20 }}>{item.gotchas.map((g, i) => <li key={i} style={{ fontSize: 13, color: "#333", lineHeight: 1.5, marginBottom: 6 }}>{g}</li>)}</ul></>)}
        {item.phase && (<><h4 style={{ fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, color: GOLD, margin: "20px 0 8px" }}>Build Phase</h4><p style={{ fontSize: 14, color: "#333" }}>{typeof item.phase === "number" ? `Phase ${item.phase}` : item.phase}</p></>)}
      </div>
    </>
  );
}
